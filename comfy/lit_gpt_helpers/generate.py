...
from comfy.lit_gpt_helpers.generate_base import generate
import json
import sys
import time
import warnings
...
def generate_comfy(
    prompt: str = "I am angry and upset, what should I do?",
    input: str = "",
    ...
    quantize: Optional[Literal["bnb.nf4", "bnb.nf4-dq",
                               "bnb.fp4", "bnb.fp4-dq", "bnb.int8", "gptq.int4"]] = None,
    ...
    if not model or not tokenizer:
        ...
        with lazy_load(checkpoint_path) as checkpoint, lazy_load(adapter_path) as adapter_checkpoint:
            checkpoint.update(adapter_checkpoint.get(
                "model", adapter_checkpoint))
            model.load_state_dict(checkpoint, strict=quantize is None)
        fabric.print(
            f"Time to load the model weights: {time.time() - t0:.02f} seconds.", file=sys.stderr)
